    ...
    <div class="bg-white dark:bg-[#1b2e4b] rounded-lg w-full max-w-md">
      ...
      <div class="flex items-center justify-between p-6 border-b border-[#e0e6ed] dark:border-[#1b2e4b]">
        <h5 class="text-lg font-semibold text-[#3b3f5c] dark:text-white-light">
          {{ isEditing ? 'Editar Tono' : 'Nuevo Tono' }}
        </h5>
        <button
          @click="$emit('close')"
      ...
      <form @submit.prevent="handleSubmit" class="p-6">
        ...
        <div class="mb-5">
          ...
          <input
            ...
            placeholder="Ingrese el nombre del tono"
            required
            maxlength="100"
          />
          <div v-if="errors.nombre" class="text-red-500 text-sm mt-1">
            <ul class="list-disc list-inside">
              <li v-for="(error, index) in errors.nombre" :key="index">
                {{ error }}
              </li>
            </ul>
          </div>
        ...
        <div class="mb-6">
          ...
          <div class="text-xs text-gray-500 mt-1">
            {{ form.activo ? 'El tono estará disponible' : 'El tono estará oculto' }}
          </div>
        </div>
...
const { loading } = storeToRefs(tonoStore);

// Estado del formulario
const form = ref<TonoForm>({
  nombre: '',
  activo: true,
});
...
// Computed
const isEditing = computed(() => !!props.tono);
...
const loadTonoData = () => {
  if (props.tono) {
    form.value.nombre = props.tono.nombre || '';
    form.value.activo = props.tono.activo;
  }
};
...
// Watcher adicional para cuando cambia la tono prop
watch(
  () => props.tono,
  (newTono) => {
    if (newTono && props.visible) {
      loadTonoData();
    }
  },
  { immediate: true }
...
const handleSubmit = async () => {
  try {
    ...
    const formData: TonoForm = {
      nombre: form.value.nombre.trim(),
      activo: form.value.activo,
    };

    if (isEditing.value && props.tono) {
      await tonoStore.updateTono(props.tono.id, formData);
    } else {
      await tonoStore.createTono(formData);
    }
