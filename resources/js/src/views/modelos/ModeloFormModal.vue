    ...
    <div class="bg-white dark:bg-[#1b2e4b] rounded-lg w-full max-w-md">
      ...
      <form @submit.prevent="handleSubmit" class="p-6">
        ...
        <div class="mb-6">
          ...
          <div class="text-xs text-gray-500 mt-1">
            {{ form.activo ? 'El modelo estará disponible' : 'El modelo estará oculto' }}
          </div>
        </div>
...
const { loading } = storeToRefs(modeloStore);

// Estado del formulario
const form = ref<ModeloForm>({
  nombre: '',
  activo: true,
});
...
// Computed
const isEditing = computed(() => !!props.modelo);
...
const loadModeloData = () => {
  if (props.modelo) {
    form.value.nombre = props.modelo.nombre || '';
    form.value.activo = props.modelo.activo;
  }
};
...
// Watcher adicional para cuando cambia la modelo prop
watch(
  () => props.modelo,
  (newModelo) => {
    if (newModelo && props.visible) {
      loadModeloData();
    }
  },
  { immediate: true }
...
const handleSubmit = async () => {
  try {
    ...
    const formData: ModeloForm = {
      nombre: form.value.nombre.trim(),
      activo: form.value.activo,
    };

    if (isEditing.value && props.modelo) {
      await modeloStore.updateModelo(props.modelo.id, formData);
    } else {
      await modeloStore.createModelo(formData);
    }
