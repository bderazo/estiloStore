    ...
    <div class="bg-white dark:bg-[#1b2e4b] rounded-lg w-full max-w-md">
      ...
      <form @submit.prevent="handleSubmit" class="p-6">
        ...
        <div class="mb-6">
          ...
          <div class="text-xs text-gray-500 mt-1">
            {{ form.activo ? 'El sabor estará disponible' : 'El sabor estará oculto' }}
          </div>
        </div>
...
const { loading } = storeToRefs(saborStore);

// Estado del formulario
const form = ref<SaborForm>({
  nombre: '',
  activo: true,
});
...
// Computed
const isEditing = computed(() => !!props.sabor);
...
const loadSaborData = () => {
  if (props.sabor) {
    form.value.nombre = props.sabor.nombre || '';
    form.value.activo = props.sabor.activo;
  }
};
...
// Watcher adicional para cuando cambia la sabor prop
watch(
  () => props.sabor,
  (newSabor) => {
    if (newSabor && props.visible) {
      loadSaborData();
    }
  },
  { immediate: true }
...
const handleSubmit = async () => {
  try {
    ...
    const formData: SaborForm = {
      nombre: form.value.nombre.trim(),
      activo: form.value.activo,
    };

    if (isEditing.value && props.sabor) {
      await saborStore.updateSabor(props.sabor.id, formData);
    } else {
      await saborStore.createSabor(formData);
    }
