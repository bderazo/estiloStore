        ...
        <div class="card border-0 shadow-sm mb-4" style="border-radius: 15px; background: var(--secondary-color-pastel);">
            <div class="card-body p-4">
                ...
                <div class="input-group">
                    <input type="text" id="sku_search" class="form-control border-0 fs-2" placeholder="Ej: ART-001..."
                        autocomplete="off">
        ...
        <div class="row">
            <div class="col-lg-8">
                                ...
                                <tr>
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <img src="{{ $item['imagen'] ? asset($item['imagen']) : asset('assets/img/no-product.png') }}"
                                                width="60" class="rounded me-3">
                                            <div>
                                                <h6 class="mb-0 fw-bold">{{ $item['nombre'] }}</h6>
                                                <small class="text-muted">{{ $item['color'] }} /
                                                    {{ $item['talla'] }}</small>
                                            </div>
                                        </div>
                                    </td>
                ...
                <div class="row g-3 mt-4">
                    <div class="col-md-3">
                        <a href="{{ isset($folletoPrincipal) ? route('folleto.descargar', $folletoPrincipal->id) : '#' }}"
                            class="card h-100 border-0 shadow-sm text-center p-3 text-decoration-none">
                            <i class="fas fa-file-pdf fa-2x text-danger mb-2"></i>
                            <span class="small fw-bold text-dark">Descargar Catálogo</span>
                        </a>
                    </div>
                    <div class="col-md-3">
                        <button type="button" onclick="pagarReserva(event)"
                            class="card h-100 border-0 shadow-sm text-center p-3 w-100 {{ !$totales['puede_pagar'] ? 'opacity-50' : '' }}"
                            {{ !$totales['puede_pagar'] ? 'disabled' : '' }}>
                ...
                <div class="card border-0 shadow-sm p-4" style="border-radius: 20px;">
                    ...
                    @if (!($totales['puede_pagar'] ?? false))
                        <div class="alert alert-warning small">
                            Faltan <strong>{{ 4 - ($totales['cantidad_items'] ?? 0) }}</strong> productos para reservar.
                        </div>
                    @endif
                </div>
            </div>
        ...
        function pagarReserva(event) {
            ...
            .then(data => {
                if (data.error) {
                    ...
                    // Ajustamos la redirección a la ruta de "Mis Pedidos" o el detalle del pedido
                    window.location.href = "{{ url('/pedidos') }}/" + data.pedido_id;
                }
            })
            .catch(error => {
